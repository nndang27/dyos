...
export class OutputParser {
    public static parseMessage(msg: any, outputAccumulator: CellOutput[], workspace: string): void {
        ...
        if (msgType === "stream") {
            ...
            if (lastOutput && lastOutput.type === "stream" && (lastOutput as any).name === name) {
            ...
            for (const [mime, value] of Object.entries(data)) {
                if (mime.startsWith("image/png")) {
                    ...
                    const base64Data = (value as string).replace(/^data:image\/png;base64,/, "");
                    const filename = `plot_${uuidv4().substring(0, 8)}.png`;
                    const filepath = path.join(workspace, filename);
                    fs.mkdirSync(workspace, { recursive: true });
                    fs.writeFileSync(filepath, base64Data, "base64");
                    formattedData["image/png"] = `file://${filepath}`;
                } else if (mime === "text/html" && (value as string).includes("<table") && (value as string).includes("dataframe")) {
                    ...
                    formattedData[mime] = (value as string);
                } else {
                    formattedData[mime] = Array.isArray(value) ? value.join("") : (value as string);
                }
            }
